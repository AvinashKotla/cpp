...
//void printmovie(moviestruct movie); // function declaration
void printmovie(moviestruct movie) // Using loops and structures to receive input from keyboard
{
	cout << movie.name;
...
int main()
{
	...
	*/
	...
	/* Using loops and structures to receive input from keyboard
	moviestruct arr[3];
	string str;
	for (int i = 0; i < 2; i++)
	{
		...
		//cin >> arr[i].year; // use of direct cin to receive year input shows some inconsistency, always use stringstream
		getline(cin, str);
		stringstream(str) >> arr[i].year; // needs <sstream> header file
		cout << "Enter the name of the lead of the movie\n";
		getline(cin, arr[i].lead);
	}
	cout << "The movie details entered are: \n";
	for (int i = 0; i < 2; i++)
	{
		printmovie(arr[i]);
	}
	*/
	...
	moviestruct mov1, arr2[3]; // object defined under the structure moviestruct
	moviestruct* pmov1; // defining a pointer to an object
	pmov1 = &mov1;
	string str;
	int len = 2;

	// pmov1->name is same as (*pmov1).name
	// *pmov1.name is same as *(pmov1.name)
	// a.b gives attribute b of object a
	// a->b gives attribute b of object pointed to by a (same as (*a).b)
	// *a.b gives value pointed to by attribute b of object a (same as *(a.b))

	for (int i = 0; i < len; i++)
	{
		cout << "Enter a movie name\n";
		getline(cin, pmov1->name); 
		cout << "Enter the release year\n";
		getline(cin,str);
		(stringstream)str >> pmov1->year;
		cout << "Enter the lead actress\n";
		getline(cin, pmov1->lead);
	}
	cout << "The movie details entered are: \n";
	for (int i = 0; i < len; i++)
	{
	...
	*/
	...
	/* dynamically take matrix size and store elements?
	int r, c;
	int* arr;
	arr = new int[][];
